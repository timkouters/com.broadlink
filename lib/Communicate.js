...
class Communicate  {
	...
	configure( options ) {
		...
		if( options.deviceId) {
			...
			this.deviceId = new Uint8Array([0,0,0,0]);
		}
		...
		/* 
		 * Fix code sending- and request-header for RM3 mini D and RM4 devices
		 */
		this.request_header = new Uint8Array([0,0]);
		this.code_sending_header = new Uint8Array([0,0]);
		if ((this.deviceType == 11) || 
		     this.deviceType == 12) 
		{
				this.request_header = new Uint8Array([0x04, 0x00]);
    			this.code_sending_header = new Uint8Array([0xda, 0x00]);
		}
		if (this.deviceId == 0x5f36)
		{
			this.code_sending_header = new Uint8Array([0xd0, 0x00]);
      		this.request_header = new Uint8Array([0x04, 0x00]);
		}
		console.log("request_header %d %d", this.request_header[0], this.request_header[1]);
	}
	...
	async sendto(packet, ipaddress, port, timeout ) {
		...
		if( this.dgramSocket === undefined ) {
			console.log("==> Communicate.sendto - create socket");
			...
			this.dgramSocket.on('message', (msg, rinfo) => {
				...
				console.log('<== Communicate.sendto - got message with size='+rsp['size']);
				console.log('<== Communicate.sendto - got message with size='+rsp['size']);
	...
	async send_packet(command, version, payload) {
		...
		console.log('->send_packet: resp='+Util.asHex(response.data))
		
		if( response ) {
			console.log('got response:'+Util.asHex(response.data))
			...
			if( response.error == 0 ) {
				...
				console.log('<-send_packet: resp.payload='+Util.asHex(response.cmd)+":"+Util.asHex(response.payload))
			}
		}
		else {
			response.error = -1
	...
	async auth() {
		...
		console.log('<== auth: auth data :'+JSON.stringify(authData));
	...
	*/
	...
	async enter_learning() {
		...
		payload[0] = 3;
		payload = Util.concatTypedArrays( payload, this.request_header );
		console.log("payload: %d%d", payload[1], payload[2]);
	...
	async _check_data( payload ) {
		...
		return new Promise( function(resolve, reject ) {
			let tm = setInterval( function() {
				console.log('_check_data - repeat='+this.checkRepeatCount)
					...
					.then ( 
						function( response ) {
							if( response.error == 0 ) {
								clearInterval(tm)
								tm = null
								console.log('resp = ' + Util.asHex(response.payload))
	...
	async send_IR_RF_data( data) {
		...
		payload[0] = 0x02
		if(this.deviceId = 0x5f36) {
			payload = Util.concatTypedArrays( payload, this.code_sending_header);
		}
